...
\begin{tikzpicture}
	...
	\filldraw (P) circle(1pt) node[ anchor = west ] {\mg{$P= a (\cos E, \sin E)$}};

	% Point Q on the ellipse (Q = (a cos theta, b sin theta))
	\coordinate (Q) at ({\semiMajor*cos(\angle)}, {\semiMinor*sin(\angle)});
	%\filldraw (Q) circle(1pt) node[ anchor = west ] {\colorbox{white}{$Q= (a \cos E, b \sin E)$}};
	%\filldraw (Q) circle(1pt) node[ anchor = west ] {$Q= (a \cos E, b \sin E)$};
	\filldraw (Q) circle(1pt) node[ anchor = west ] {$Q\colorbox{white}{=} (a \cos \nu, b \sin 
	\nu)$};
	...
	\node at (-1,4.65) [ left ] {reference circle};
	\node at (-1.4, 4.1) [ left ] {$\xi^{2} +\eta^{2} = a^{2}$};
	\node at (1.5, 4.65) [ right ] {orbit ellipse};
	\node at (1.5, 4.1) [ right ] {$\displaystyle\frac{\xi^{2}}{a^{2}} + \frac{\eta^{2}}{b^{2}} = 1$};

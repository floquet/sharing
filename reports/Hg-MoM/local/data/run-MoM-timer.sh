...
export stem="sphere-d005"
export myFile="/Tlaloc/Mercury-MoM/times/time-${stem}-centos.txt"
date > ${myFile}
pwd >> ${myFile}
echo "" >> ${myFile}
...
for index in {01..08}; do
    dirOutput="/Tlaloc/Mercury-MoM/library/sphere/005m/MoM/resolution-${index}"
    mkdir -p ${dirOutput}
    export indexSECONDS=${SECONDS}
    export facetfilename="${stem}-${index}"
    # radar frequencies"
    for frequency in {03..30}; do

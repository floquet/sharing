...
!   RCStable: table of mean total RCS ( nu, alpha )
    ...
    type :: meanRCS
        ! rank 2: triangles
        real ( kind = rp ), allocatable :: profile ( : , : )
        ...
        ! scalar
        integer ( kind = ip ) :: nu_count = 0, yaw_count = 0
        character ( kind = kindA, len =  64 ) :: quarry = ""
        character ( kind = kindA, len = 512 ) :: file_name = ""
        character ( kind = kindA, len =  22 ) :: about = "mean Total RCS in sq m"
...
contains
    ...
    subroutine read_profile_sub ( me, file_name, quarry, numRows, numColumns )
        ...
        character ( kind = kindA, len = * ), intent ( in ) :: file_name, quarry
        integer ( kind = ip ),                      intent ( in ) :: numRows, numColumns
        ! locals
        !integer ( kind = ip ) :: k = 0, nu = 0, nu_index = 0
    ...
    subroutine read_data_file_sub ( me )
        class ( meanRCS ), target :: me
        integer ( kind = ip ) :: u_data = 0, row = 0, column = 0

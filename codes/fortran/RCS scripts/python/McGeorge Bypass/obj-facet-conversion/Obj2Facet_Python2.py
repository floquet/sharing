...
with io.open(outputFileName, 'w', encoding='utf-8') as outputFile:
    outputFile.write(unicode('FACET FILE V3.4 '))
    outputFile.write(unicode(datetime.today().strftime('%d-%b-%Y %H:%M:%S')))
    outputFile.write(unicode('\n'))

    outputFile.write(unicode(DEFAULT_PART_COUNT))
    outputFile.write(unicode('\n'))
    outputFile.write(unicode(DEFAULT_PART_NAME))
    outputFile.write(unicode('\n'))
    outputFile.write(unicode(DEFAULT_PART_MIRROR))
    outputFile.write(unicode('\n'))

    outputFile.write(unicode(str(vertexCount)))
    outputFile.write(unicode('\n'))
    outputFile.write(unicode(vertexLines))

    outputFile.write(unicode(DEFAULT_SUBPART_COUNT))
    outputFile.write(unicode('\n'))
    outputFile.write(unicode(DEFAULT_SUBPART_NAME))
    outputFile.write(unicode('\n'))

    outputFile.write(unicode(DEFAULT_ELEMENT_DESCRIPTION.format(facetCount)))
    outputFile.write(unicode('\n'))
    outputFile.write(unicode(facetLines))

...
module mIOtools
    ...
    implicit none
    ! parameters
    integer ( ip ),        parameter :: fnl = fileNameLength, msgl = messageLength, oll = objLineLength
    character ( len = * ), parameter :: moduleCrash = "Program crashed in module 'mIOtools', "
    ...
    type :: textLines
        ...
        integer ( ip ),          allocatable :: linesVertex ( : ), linesFace ( : )
        ! rank 0
...
contains
    ...
    subroutine classify_vertex_or_face_sub ( me, numFaces, numVertices )
        class ( textLines ), target    :: me
        integer ( ip ), intent ( out ) :: numVertices, numFaces
        integer ( ip ) :: kLines = 0
    ...
    subroutine file_closer_sub ( io_unit, fileName, crashChain, optionalMessage )
        ...
        character ( len = * ),           intent ( in ) :: crashChain
        character ( len = * ), optional, intent ( in ) :: optionalMessage
        character ( len = msgl ) :: subroutineCrash

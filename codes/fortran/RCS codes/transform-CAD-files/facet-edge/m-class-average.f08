...
module mClassAverage
    ...
    type :: extremal
        ...
        real    ( kind = rp ) :: maxValue = - huge( 1.0_rp ), minValue = huge( 1.0_rp ), variation = 0.0_rp
        integer ( kind = ip ) :: maxLoc = 0, minLoc = 0
    end type extremal
    ...
    type :: average
        ...
        real ( kind = rp ) :: mean = 0.0_rp, mean2 = 0.0_rp, standardDeviation = 0.0_rp
...
contains
    ...
    subroutine write_statistics_sub ( me, io_unit, propertyName, census )
        ...
        integer ( kind = ip ), intent ( in ) :: io_unit, census
        character ( kind = kindA, len = * ), intent ( in ) :: propertyName
            write ( io_unit , fmt = '( 4g0 )' ) "Statistics for ", census, " ", propertyName
    ...
    subroutine compute_mean_and_variance_sub ( me, vector, one )
        class ( average ),  target :: me
        real ( kind = rp ), intent ( in ) :: vector ( : ), one ( : )
       !real ( kind = rp ) :: clicks = 0.0_rp, threshold = 0.0_rp, x = 0.0_rp
        real ( kind = rp ) :: x = 0.0_rp, relativeError = 0.0_rp
            !clicks    = 10.0_rp
            !threshold = clicks * epsilon ( 1.0_rp )
            relativeError = 0.000001_rp ! rough single precision
            ...
            if ( x < 0.0_rp ) then
                !if ( abs ( x /  me % mean2 ) > threshold ) then
                if ( abs ( x /  me % mean2 ) > relativeError ) then
                    write ( * , fmt = '( 2g0 )' ) "Program trying to compute the square root of a negative number x = ", x
                   !write ( * , fmt = '( 5g0 )' ) "Relative error tolerance is ", clicks, " x machine epsilon = ", threshold, "."
                    write ( * , fmt = '( 3g0 )' ) "Relative error tolerance is ", relativeError, "."
                    write ( * , fmt = '(  g0 )' ) "This error may have occured when taking the difference of two large numbers."
                    write ( * , fmt = '( 4g0 )' ) "Mean of the squares = ", me % mean2, "; square of the mean = ", me % mean ** 2
                    write ( * , fmt = '( 4g0 )' ) "Difference = ", x, "; relative error = ", abs ( x /  me % mean2 )

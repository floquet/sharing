...
module mClassFace
    ...
    character ( kind = kindA, len = * ), parameter :: moduleCrash = "Program crashed in module 'mClassFace', "
    ...
    type :: face
        ! rank 1: triangles
        integer ( kind = ip ) :: indexVertex ( 1 : dim ) = zero_vector_i
        ! rank 0
        real ( kind = rp ) :: area = zero
...
contains
    ...
    subroutine computeFaceArea_sub ( me, myVertices )
        ...
        ! rank 1
        real ( kind = rp ) :: x ( 1 : dim ) = zero_vector_r, y ( 1 : dim ) = zero_vector_r, z ( 1 : dim ) = zero_vector_r
        ! rank 0
        real ( kind = rp ) :: s = zero, a = zero, b = zero, c = zero

...
module mFacetWriter
    ...
    ! utilities
    use mFormatDescriptors,             only : fmt_one, fmt_two, fmt_stat, fmt_iomsg
    ...
    character ( kind = kindA, len = * ), parameter :: moduleCrash = "Program crashed in module 'mFacetWriter', "
...
contains
    ...
    subroutine writeFaces ( io_facet, faces )
        ...
        integer ( kind = ip ), intent ( in ) :: io_facet
        integer ( kind = ip ) :: k = 0, numFaces = 0
    ...
    subroutine writeHeaderFace ( io_facet, numSubParts, subPartName, numFaces )
        ...
        integer ( kind = ip ),        intent ( in ) :: io_facet, numSubParts, numFaces
        character ( kind = kindA, len = * ), intent ( in ) :: subPartName
        character ( len = *  ), parameter :: crashChain = moduleCrash // trim ( "subroutine 'writeHeaderFace'." )
            ...
            write ( unit = io_facet , fmt = '( "<", g0, ">" )', iostat = io_stat, iomsg = io_msg ) trim ( subPartName )
    ...
    subroutine writeVertices ( io_facet, vertices )
        ...
        integer ( kind = ip ),  intent ( in ) :: io_facet
        integer ( kind = ip ) :: k = 0, numVertices = 0
        character ( kind = kindA, len = * ), parameter :: crashChain = moduleCrash // trim ( "subroutine 'writeVertices'." )
    ...
    subroutine writeHeaderVertex ( io_facet, numParts, partName, mirror )
        ...
        integer ( kind = ip ),        intent ( in ) :: io_facet, numParts, mirror
        character ( kind = kindA, len = * ), intent ( in ) :: partName
        ! containers for date and time
        integer ( kind = ip ) :: dt_values ( 1 : 8 ) = 0
        ...
        character ( len = *  ), parameter :: crashChain = moduleCrash // trim ( "subroutine 'writeHeaderVertex'." )
            ...
            write ( unit = io_facet , fmt =  '( "<", g0, ">" )', iostat = io_stat, iomsg = io_msg ) trim ( partName )
    ...
    subroutine local_iostat_check_sub ( message1, message2, crashChain )
        character ( kind = kindA, len = * ), intent ( in )           :: message1, crashChain
        character ( kind = kindA, len = * ), intent ( in ), optional :: message2
            if ( io_stat /= 0 ) then

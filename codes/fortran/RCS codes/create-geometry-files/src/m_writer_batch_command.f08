...
contains
    ...
    subroutine updateNameList ( myQuarry, unit, angle )
        type ( quarry ), intent ( in ) :: myQuarry
        integer,         intent ( in ) :: angle, unit

            write ( unit, fmt = '( 3g0 )' ) trim ( myQuarry % momFile ), ", ", angle

        return
    end subroutine updateNameList


    subroutine updateHarvestScript ( myQuarry, unit, angle )
        type ( quarry ), intent ( in ) :: myQuarry
        integer,         intent ( in ) :: angle, unit
        character ( len = 256 ) :: cmd = ""

            write ( unit, fmt = '(  g0 )' ) "export ticks=${SECONDS}"
            write ( cmd,  fmt = '( 5g0 )' ) 'new_step "./sweeper ', trim ( myQuarry % momFile ), ' ', angle, '"'
            write ( unit, fmt = '(  g0 )' ) trim ( cmd )
            write ( unit, fmt = '(  g0 )' ) 'echo ""'
            write ( cmd,  fmt = '( 3g0 )' ) 'echo "time used to run ', trim ( myQuarry % momFile ), ' = $((SECONDS-ticks)) s"'
            write ( unit, fmt = '(  g0 )' ) trim ( cmd )
            write ( unit, fmt = '(  g0 )' ) 'echo ""'
        ...
        return
    end subroutine updateHarvestScript
    ...
    subroutine purgeSupportFiles ( myQuarry, unit )
        ...
        character ( len = 256 ) :: cmd = ""
            ...
            call closeTimer ( unit )

        return
    end subroutine purgeSupportFiles

    subroutine writeBashHeader ( nameAndPath, header, io )
    ...
    end subroutine writeBashHeader

    subroutine writeBashCounters ( io )
        ...
        return
    end subroutine writeBashCounters

    subroutine annotateMoMdriverScript ( io )
        integer, intent ( in ) :: io
        ...
        return
    end subroutine annotateMoMdriverScript

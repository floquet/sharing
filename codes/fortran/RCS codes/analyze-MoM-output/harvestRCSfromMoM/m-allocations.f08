...
module mAllocations
    ...
    type :: allocationToolKit
        real    ( rp ) :: requestedGB = 0.0_rp
        integer ( ip ) :: alloc_status = 0_ip, sizeElementBits = 0_ip, numElements = 0_ip
        character ( len = fnl ) :: alloc_message = "", myKind = ""
    contains
            ! rank 3
        procedure, public :: allocate_rank_three_reals    => allocate_rank_three_reals_sub
            ! rank 2
        procedure, public :: allocate_rank_two_reals      => allocate_rank_two_reals_sub
            ! rank 1
        procedure, public :: allocate_rank_one_characters => allocate_rank_one_characters_sub
        procedure, public :: allocate_rank_one_integers   => allocate_rank_one_integers_sub
        procedure, public :: allocate_rank_one_reals      => allocate_rank_one_reals_sub
            ! service routines
        procedure, public :: compute_requested_gb         => compute_requested_gb_sub
        procedure, public :: post_error_message_intrinsic => post_error_message_intrinsic_sub
        procedure, public :: post_error_message_composite => post_error_message_composite_sub
        procedure, public :: compute_number_of_elements   => compute_number_of_elements_sub
        procedure, public :: compute_number_of_elements2  => compute_number_of_elements2_sub
    end type allocationToolKit

    private :: compute_requested_gb_sub, compute_number_of_elements_sub, compute_number_of_elements2_sub
    private :: post_error_message_intrinsic_sub, post_error_message_composite_sub
    private :: allocate_rank_one_integers_sub, allocate_rank_one_characters_sub
    private :: allocate_rank_one_reals_sub, allocate_rank_two_reals_sub, allocate_rank_three_reals_sub
...
contains
    ...
    subroutine allocate_rank_one_characters_sub ( me, character_array, index_min, index_max )
        ...
        character ( len = * ), allocatable, intent ( out ) :: character_array ( : )
    ...
    subroutine allocate_rank_three_reals_sub ( me, rank_3_real_array, dim1_index_min, dim1_index_max, &
                                                                      dim2_index_min, dim2_index_max, &
                                                                      dim3_index_min, dim3_index_max )
        class ( allocationToolKit ),      target :: me
        real ( rp ), allocatable, intent ( out ) :: rank_3_real_array ( : , : , : )
        integer ( ip ),           intent ( in  ) :: dim1_index_min, dim1_index_max, &
                                                    dim2_index_min, dim2_index_max, &
                                                    dim3_index_min, dim3_index_max

            call compute_number_of_elements3_sub ( me, dim1_index_min = dim1_index_min, dim1_index_max = dim1_index_max, &
                                                       dim2_index_min = dim2_index_min, dim2_index_max = dim2_index_max, &
                                                       dim3_index_min = dim3_index_min, dim3_index_max = dim3_index_max )
            ! how much memory is requested?
            me % sizeElementBits = storage_size ( [ 1.0_rp, 1.0_rp, 1.0_rp ] )
            call compute_requested_gb_sub ( me )
            write ( me % myKind, fmt = '( "data type is rcs_table_rank_3" )' )

            ! hygiene check: is the array already allocated?
            if ( allocated ( rank_3_real_array ) ) then
                deallocate ( rank_3_real_array, stat = me % alloc_status, errmsg = me % alloc_message )
                if ( me % alloc_status /= 0 ) then
                    call post_error_message_intrinsic_sub ( me, action = 'deallocating' )
                end if
            end if
            ...
            ! the compiler may allow allocations which are not possible
            allocate ( rank_3_real_array ( dim1_index_min : dim1_index_max, &
                                           dim2_index_min : dim2_index_max, &
                                           dim3_index_min : dim3_index_max ), &
                        stat = me % alloc_status, errmsg = me % alloc_message )
            if ( me % alloc_status /= 0 ) then
                call post_error_message_intrinsic_sub ( me, action = 'allocating' )
            end if

            ! verify the memory is truly available
            rank_3_real_array ( : , : , : ) = 0.0_rp

    end subroutine allocate_rank_three_reals_sub

    subroutine allocate_rank_two_reals_sub ( me, rank_2_real_array, dim1_index_min, dim1_index_max, dim2_index_min, dim2_index_max )
        class ( allocationToolKit ), target :: me
        real ( rp ), allocatable, intent ( out ) :: rank_2_real_array ( : , : )
        integer ( ip ),           intent ( in  ) :: dim1_index_min, dim1_index_max, dim2_index_min, dim2_index_max

            call compute_number_of_elements2_sub ( me, dim1_index_min = dim1_index_min, dim1_index_max = dim1_index_max, &
                                                       dim2_index_min = dim2_index_min, dim2_index_max = dim2_index_max )
            ! how much memory is requested?
            me % sizeElementBits = storage_size ( [ 1.0_rp, 1.0_rp ] )
            call compute_requested_gb_sub ( me )
            write ( me % myKind, fmt = '( "data type is electricFields" )' )

            ! hygiene check: is the array already allocated?
            if ( allocated ( rank_2_real_array ) ) then
                deallocate ( rank_2_real_array, stat = me % alloc_status, errmsg = me % alloc_message )
                if ( me % alloc_status /= 0 ) then
                    call post_error_message_intrinsic_sub ( me, action = 'deallocating' )
                end if
            end if
            ...
            ! the compiler may allow allocations which are not possible
            allocate ( rank_2_real_array ( dim1_index_min : dim1_index_max, dim2_index_min : dim2_index_max ), &
                        stat = me % alloc_status, errmsg = me % alloc_message )
            if ( me % alloc_status /= 0 ) then
                call post_error_message_intrinsic_sub ( me, action = 'allocating' )
            end if
            ...
            ! verify the memory is truly available
            rank_2_real_array ( : , : ) = 0.0_rp

    end subroutine allocate_rank_two_reals_sub

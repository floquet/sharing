...
module mSolutionNormal
    ...
    type :: solutionNormal
        ...
        real ( rp ), allocatable :: Winv  ( : , : )
        real ( rp ), allocatable :: Wtemp ( : , : )
        ! A is m x n
        ! data vectors length m
        real ( rp ), allocatable :: Asb ( : )
        ! data vectors length n ( = d + 1 )
        real ( rp ), allocatable :: amplitude   ( : )
        real ( rp ), allocatable :: error       ( : )
        real ( rp ), allocatable :: residual    ( : )
        real ( rp ), allocatable :: signalNoise ( : )

        ! rank 0
        real( rp ) :: sse

        ! scalar
        integer ( ip ) :: len_data = 0
        integer ( ip ) :: len_solution = 0
        integer ( ip ) :: order_of_fit = 0
        character ( len =  64 ) :: tag = ""
        character ( len =  64 ) :: method = ""
        character ( len =  64 ) :: quarry = ""
        character ( len =  64 ) :: fit_type = ""
        character ( len = 512 ) :: data_source = ""
    contains
...
contains
    ...
    subroutine normal_equations_solve_sub ( me, mySystem )
        ...
        type ( systemMatrix ), intent ( in ) :: mySystem
        ! locals
        real ( rp )    :: lapackWorkArray ( 1 : me % len_solution )
        real ( rp )    :: factor = 0
        integer ( ip ) :: pivotIndices    ( 1 : me % len_solution )
        integer ( ip ) :: rows = 0, cols = 0
        integer ( ip ) :: info = 0, lda = 0, lwork = 16
        integer ( ip ) :: k = 0
        ! LWORK is INTEGER
        ...
        ! message related to LWORK is issued by XERBLA.
            ...
            cols = size ( me % W ( : , : ), 2 )
            lda = rows
            write ( * , * ) "into dgetrf: rows = ", rows, ", cols = ", cols
            ! lda > max ( 1, rows )
            lda = rows + 1

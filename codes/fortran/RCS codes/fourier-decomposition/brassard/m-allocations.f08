...
module mAllocations
    ...
    use mLibraryConstants,              only : fileNameLength, oneGB, zero
    ...
    type :: allocationToolKit
        ...
        integer   ( kind = ip ) :: numRows = 0_ip, numCols = 0_ip
        character ( kind = kindA, len = fnl ) :: alloc_message = "", myKind = ""
    contains
            ! rank 2
        procedure, public :: allocate_rank_two_reals       => allocate_rank_two_reals_sub, &
                             clone_rank_two_reals          => clone_rank_two_reals_sub
            ! rank 1
        procedure, public :: allocate_rank_one_characters  => allocate_rank_one_characters_sub, &
            ...
            ! service routines
        procedure, public :: compute_requested_gb_rank_one => compute_requested_gb_rank_one_sub, &
                             compute_requested_gb_rank_two => compute_requested_gb_rank_two_sub, &
    ...
    private :: allocate_rank_one_integers_sub, allocate_rank_one_characters_sub, allocate_rank_one_reals_sub, &
               clone_rank_one_reals_sub, clone_rank_two_reals_sub, &
               compute_requested_gb_rank_one_sub, compute_requested_gb_rank_two_sub, &
               compute_number_of_elements_sub, compute_number_of_elements2_sub, &
...
contains
    ...
    subroutine clone_rank_two_reals_sub ( me, target_array, mold_array )
        class ( allocationToolKit ), target :: me
        real ( kind = rp ), allocatable, intent ( out ) :: target_array ( : , : )
        real ( kind = rp ),              intent ( in  ) :: mold_array   ( : , : )

            me % numRows = size ( target_array, 1 )
            me % numCols = size ( target_array, 2 )
            ! how much memory is requested?
            me % sizeElementBits = storage_size ( 1.0_rp )
            call compute_requested_gb_rank_two_sub ( me )
            write ( me % myKind, '( "data type is REAL ( kind = rp )" )' )
            ...
            target_array ( : , : ) = zero
        return
    end subroutine clone_rank_two_reals_sub
    ...
    subroutine clone_rank_one_reals_sub ( me, target_array, mold_array )
        ...
        real ( kind = rp ),              intent ( in  ) :: mold_array ( : )
            ...
            ! how much memory is requested?
            me % sizeElementBits = storage_size ( 1.0_rp )
            call compute_requested_gb_rank_one_sub ( me )
            write ( me % myKind, '( "data type is REAL ( kind = rp )" )' )
    ...
    subroutine allocate_rank_one_integers_sub ( me, integer_array, index_min, index_max )
        ...
        integer ( kind = ip ),              intent ( in  ) :: index_min, index_max
            ...
            call compute_requested_gb_rank_one_sub ( me )
            write ( me % myKind, '( "data type is INTEGER ( kind = ip )" )' )
    ...
    subroutine allocate_rank_two_reals_sub ( me, real_array, numRows, numColumns )
        class ( allocationToolKit ), target :: me
        ! tourists
        real    ( kind = rp ), allocatable, intent ( out ) :: real_array ( : , : )
        integer ( kind = ip ),              intent ( in  ) :: numRows, numColumns
        ! locals
        integer ( kind = ip ) :: numElements

            numElements = numRows * numColumns

            ! how much memory is requested?
            me % sizeElementBits = storage_size ( 1.0_rp )
            me % requestedGB     = real ( me % sizeElementBits * numElements, rp ) / oneGB
            write ( me % myKind, '( "data type is REAL ( kind = rp )" )' )
            ...
            if ( allocated ( real_array ) ) then
                deallocate ( real_array, stat = me % alloc_status, errmsg = me % alloc_message )
                if ( me % alloc_status /= 0 ) then
                    call post_error_message_intrinsic_sub ( me, 'deallocating' )
                end if
            end if

            ! the compiler may allow allocations which are not possible
            allocate ( real_array ( 1 : numRows, 1 : numColumns ), stat = me % alloc_status, errmsg = me % alloc_message )
            if ( me % alloc_status /= 0 ) then
                call post_error_message_intrinsic_sub ( me, 'allocating' )
            end if
    ...
    subroutine compute_requested_gb_rank_two_sub ( me )
        class ( allocationToolKit ), target :: me
            me % requestedGB  = real ( me % sizeElementBits * me % numRows * me % numCols, kind = rp ) / oneGB
        return
    end subroutine compute_requested_gb_rank_two_sub

    subroutine compute_requested_gb_rank_one_sub ( me )
        class ( allocationToolKit ), target :: me
            me % requestedGB  = real ( me % sizeElementBits * me % numElements, kind = rp ) / oneGB
        return
    end subroutine compute_requested_gb_rank_one_sub

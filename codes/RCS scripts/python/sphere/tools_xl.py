...
import numpy as np      # array gymnastics
import pandas as pd     # read TSV data *.4112.dat
...
def xl_new_workbook( testObject ):
    ...
    print ( "creating %s " % testObject.outputPathFile )
    MoMresults = xlsxwriter.Workbook( testObject.outputPathFile )

    xl_sheet_master    ( MoMresults, testObject )    # MoM summary
    xl_add_chart       ( MoMresults, testObject ) # MoM summary
...
def xl_add_chart( this_workbook, testObject ):
    # Create a new chart object.
    s = this_workbook.add_chartsheet( 'RCS plot' )
    chart = this_workbook.add_chart( {'type': 'scatter'} )
    chart.add_series( {
    'categories': '=\'' + testObject.masterSheet + '\'$B$19:$AC$19',
    'values':     '=\'' + testObject.masterSheet + '\'!$B$20:$AC$20' } )

    chart.set_title ( {'name': 'RCS for d = ' + str ( testObject.sizeValue ) + ' m sphere, resolution = ' + str ( testObject.resolution )} )
    chart.set_x_axis( {'name': 'Circumference in wavenumbers'} )
    chart.set_y_axis( {'name': 'Normalized RCS'} )
    chart.set_legend( {'delete_series': [0, 0]} )
    s.set_chart( chart )
    #s.insert_chart('D2', chart)
    return
...
def xl_add_data_sheets( this_workbook, testObject ):
    ...
    for index in range( 1, 29 ):
        ...
        # read in data file
        filename = testObject.sourcePath +  testObject.stemName + '-' + testObject.resolution + '-' + str( index + 2 ).zfill(2) + '.4112.dat.txt'
        # if the file does not exist, skip to the next case
        # print ( "looking for %s" % filename )
        # print ( "os.path.isfile( filename ) %s" % os.path.isfile( filename ) )
        if os.path.isfile( filename ):
            ...
            for line in range( 0, len ( data_np ) ):
                ...
                s.write( row, col, data_np[ line ][ 0 ], number_format )     # azimuth
                s.write( row, col + 1, data_np[ line ][ 1 ], number_format ) # HH
                s.write( row, col + 2, data_np[ line ][ 2 ], number_format ) # VV
                s.write( row, col + 3, data_np[ line ][ 3 ], number_format ) # HV
                s.write( row, col + 4, data_np[ line ][ 4 ], number_format ) # Vh
                row += 1
    return
...
def xl_computation ( wsheet, row, col, nu, number_format ):
    ...
    cell = xl_rowcol_to_cell ( row - 1, col )
    wsheet.write_formula ( row, col, '= 1 - size_optical_sq_m / ' + cell, number_format ); row += 2

    # circumference in wavelengths  = 2 * PI() * B11
    cell = xl_rowcol_to_cell ( row - 8, col )
    wsheet.write_formula ( row, col, '= 2 * pi() * ' + cell, number_format ); row += 1
    # normalized RCS  = B16 / size_optical_sq_m
    cell = xl_rowcol_to_cell ( row - 4, col )
    wsheet.write_formula ( row, col, '= ' + cell + ' / size_optical_sq_m', number_format )
...
def xl_set_label_column( wbook, testObject, row, col ):
    ...
    wbook.define_name( 'radius',            '=' + str( testObject.sizeValue / 2 ) )
    ...
    s.write( row, col, 'rel error (sq m)', format_label ); row += 2

    s.write( row, col, 'circumference in wavelengths', format_label ); row += 1
    s.write( row, col, 'normalized RCS', format_label )

...
contains
    ...
    subroutine getVertices ( u_obj, array, locators )
        ...
        integer ( ip ) :: j = 0, k = 0
            ...
            if ( io_stat /= 0 ) then
                write ( * , fmt_one ) "Error rewinding *.obj file to gather vertex dat"
                write ( * , fmt_stat )  io_stat
                write ( * , fmt_iomsg ) io_msg
                stop "Program crashed in subroutine getVertices..."
            endif

            do k = 1, locators % first - 1
                read ( unit = u_obj, fmt = *, iostat = io_stat,  iomsg = io_msg )
                if ( io_stat /= 0 ) then
                    write ( * , fmt_one ) "Error while positioning read pointersa"
                    write ( * , fmt_stat )  io_stat
                    write ( * , fmt_iomsg ) io_msg
                    stop "Program crashed in subroutine getVertices..."
                endif
    ...
    subroutine getFaces ( u_obj, array, locators )
        ...
        character ( len = 128 ) :: buffer = ""
            ...
            if ( io_stat /= 0 ) then
                ...
                write ( * , fmt_stat )  io_stat
                write ( * , fmt_iomsg ) io_msg
                stop "Program crashed in subroutine getFaces..."
            endif

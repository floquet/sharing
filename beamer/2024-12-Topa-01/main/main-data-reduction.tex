% typeset: Pdftex
% Afterwards compile with pdflatex > bibtex > pdflatex > pdflatex.
% in TeXShop preferences, changed edit from bibtex to biber
% beamer likes biber
% latex likes bibtex

% https://tex.stackexchange.com/questions/270633/beamer-and-the-pause-command
% https://tex.stackexchange.com/questions/1423/is-there-a-nice-way-to-compile-a-beamer-presentation-without-the-pauses
%\documentclass[handout]{beamer}
% https://tex.stackexchange.com/tags/accents/info
%
\documentclass[]{beamer}
\usepackage{pifont}
\newcommand{\cmark}{\bl{\ding{51}}}
\newcommand{\xmark}{\rd{\ding{55}}}
\usepackage{transparent}
\usepackage{epstopdf} % Automatic EPS to PDF conversion
% Fetch home directory
\usepackage{url}
\usepackage{tikz}
\usepackage{tikz-uml}
\usepackage{pgfplots}
\pgfplotsset{compat=1.17} % Ensure compatibility with PGFPlots
\usepackage{algorithm}
\usepackage{algpseudocode}
\usepackage{cancel}
% beaver vs listing styles
\usepackage{listings}
%\usepackage{minted}
\renewcommand{\ttdefault}{cmtt} % Ensure Computer Modern monospaced font
\lstset{basicstyle=\ttfamily\tiny} % Use tiny monospaced font for Beamer
 
\usepackage{tcolorbox}
\definecolor{mygray}{cmyk}{0, 0, 0, 0.5} % 50% black
\usepackage{catchfile}
\CatchFileDef{\HomePath}{|kpsewhich -var-value=HOME}{}

% Strip trailing spaces from HomePath
\makeatletter
\edef\HomePath{\expandafter\zap@space\HomePath \@empty}
\makeatother

% Define base paths
% relies on symlink  at $HOME, e.g.
% 	GitHub -> /Users/dantopa//repos-xiuhcoatal/github
\newcommand{\pGithub}			{\HomePath/GitHub/}
\newcommand{\pGithubSharing}	{\pGithub/sharing/}
\newcommand{\pGlobal}			{\pGithubSharing/global/}
\newcommand{\pGlobalSetup}		{\pGlobal/setup-global/}
% Load Additional Setup Files
	\input{\pGlobalSetup/setup-global-beamer.tex}
	\input{\pLocalSetup/setup-local.tex}
	\input{\pGlobalSetup/listings-global.tex}

% Bibliography
	%\input{\pGlobalSetup packages-global-bibliography-charlie.tex}
\addbibresource{\pShareBibliographies/radar.bib} 

\newcommand{\fourier}		{\color{blue}{ e^{i n \theta} }} 
\newcommand*{\theorembreak}{\usebeamertemplate{theorem end}\framebreak\usebeamertemplate{theorem begin}}
\newcommand{\stl} 		{\href{https://en.wikipedia.org/wiki/STL\_(file\_format)}{\bl{\texttt{*.stl}}}}
\newcommand{\obj} 		{\href{https://en.wikipedia.org/wiki/Wavefront\_.obj\_file}{\bl{\texttt{*.obj}}}}
\newcommand{\mtl} 		{\href{https://www.loc.gov/preservation/digital/formats/fdd/fdd000508.shtml}{\texttt{*.mtl}}}
%
\newcommand{\facet }	{\bl{\texttt{*.facet}}}
\newcommand{\geo} 	{\bl{\texttt{*.geo}}}
\newcommand{\lib} 	{\bl{\texttt{*.lib}}}

\setbeamercovered{transparent=10} % pause activated text
\usepackage{transparent}
\usepackage{seqsplit}

\title[Satellite Radar Cross Sections]{Data Reduction for Modelling\\ Satellite Radar Cross Sections 1/2}

\author[Daniel Topa]{\TopaHII \\ \TopaHIIEmail}
\institute{\missiontech} 
%\medskip

\date{\today}
%\date{2018-08-31}

\begin{document}

\begin{frame}
	\titlepage
\end{frame}
	%\include{\pSections "sec splash"}

	\input{\pLocalSlides "slide-Models-Radar-Cross-Sections-for-Satellites"}

\begin{frame}\frametitle{Overview}
	\tableofcontents[hideallsubsections]
\end{frame}

% main sections
	\input{\pSections "sec-overview.tex"}
	\input{\pSections "sec-input.tex"}
	%\input{\pSections "sec-running"}
	%\input{\pSections "sec-results"}
	%\input{\pSections "sec-mesh"}
	%\input{\pSections "sec-model"}
	%\appendix
	%\input{\pSections "sec-output.tex"}
	\input{\pSections "sec-existing.tex"}
	\input{\pSections "sec-backup.tex"}
	%\input{\pSections "sec-data-reduction"}
	%\input{\pSections "sec-output"}

{\tiny{
%\begin{frame}[allowframebreaks,shrink=10]\frametitle{Bibliography}
\begin{frame}[allowframebreaks]\frametitle{Bibliography}
	%\nocite{*}
	\printbibliography
\end{frame}}}

% info slide during questions
\begin{frame}
	\titlepage
\end{frame}

\end{document}

%\tiny
%\scriptsize
%\footnotesize
%\small
%\normalsize
%\large
%\Large
%\LARGE
%\huge
%\Huge

%\, thin space (normally 1/6 of a quad);
%\> medium space (normally 2/9 of a quad);
%\; thick space (normally 5/18 of a quad);

\begin{frame}\frametitle{Frame Title}
\begin{enumerate}
	\item 
	\item 
	\item 
\end{enumerate}
\end{frame}

\begin{frame}\frametitle{Frame Title}
\begin{equation}
	\begin{array}{ccc} 
			%
			%
			%
	\end{array}
%\label{eq:}
\end{equation}
\end{frame}

\begin{frame}\frametitle{ }
\center
	\href{url}{
	\begin{overpic}[ scale = 1.0 ]
	{\pLocalGraphics graphic-file}
		%\put(-7,-10){Auxiliary text.}
	\end{overpic}}
\end{frame}

\begin{frame}\frametitle{Frame Title}
\begin{table}[htp]
%\caption{default}
\begin{center}
	\begin{tabular}{cc}
		%
		%
		%
	\end{tabular}
\end{center}
%\label{tab:label}
\end{table}%
\end{frame}

